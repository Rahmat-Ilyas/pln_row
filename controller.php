...
if (isset($_POST['req'])) {
	...
	if ($_POST['req'] == 'arsipInspeksi') {
		$priode_id = $_POST['priode_id'];
		$priode = mysqli_query($conn, "SELECT * FROM tb_priode_laporan WHERE id='$priode_id'");
		$prd = mysqli_fetch_assoc($priode);
		$priode_mulai = strtotime($prd['tanggal_mulai']);
		$priode_akhir = strtotime($prd['tanggal_akhir']);
